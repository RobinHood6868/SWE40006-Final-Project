...
- **Monitoring** via PM2 (app-level) and AWS CloudWatch alarms (server-level)
...
_How can a small development team implement a zero-cost, fully automated CI/CD pipeline that ensures code quality through testing, deploys reliably to production, and provides visibility into application health?_
...
| 4   | Monitor performance    | Real-time CPU and memory stats via PM2 + CloudWatch alarms         |
...
const [cart, setCart] = useState(() => {
  try {
    return JSON.parse(localStorage.getItem("volta_cart") || "[]");
  } catch {
    return [];
  }
});
...
| 2     | Production server instrumentation                       | ✅     | PM2 (app-level) + CloudWatch alarms (server-level) |
...
### 10.4 Scope Adjustments & Future Improvements

While the core project rubric was fully achieved, certain industry-standard practices were intentionally scoped out to prioritize infrastructure quality and align with project constraints:

#### 1. HTTPS via Let's Encrypt + Nginx

- **Difficulty:** Medium
- **Why we skipped it:** Implementing full HTTPS requires purchasing a registered domain name (DuckDNS will not work seamlessly with standard Let's Encrypt certs without complex DNS challenges). This breaks the strictly "free" constraint established for the project. Our current DuckDNS HTTP setup is perfectly sufficient for the assignment rubric.

#### 2. Rolling Deployments (PM2 Cluster Mode)

- **Difficulty:** Easy to configure, but physically impossible to execute.
- **Why we skipped it:** PM2's cluster mode relies on spawning multiple application instances across multiple CPU cores. The AWS Free Tier t2.micro instance only possesses 1 vCPU. If we attempt a rolling restart on a single-core machine, we will likely just crash the server due to CPU bottlenecking. Our current explicit restart method is the safest approach for our hardware.

#### 3. Staging Environment

- **Difficulty:** Medium (Requires duplicating the pipeline and infrastructure).
- **Why we skipped it:** The rubric explicitly asks for a "test server, and a production server." We have already fulfilled this using GitHub Actions (Test) and EC2 (Production). Adding a staging environment is overkill for the university requirements and risks exhausting our free-tier AWS compute hours.

#### 4. End-to-End (E2E) Browser Tests

- **Difficulty:** Hard / Time-Consuming
- **Why we skipped it:** Writing reliable E2E tests using tools like Cypress or Playwright takes significant time, and running them in GitHub Actions headless browsers is notoriously tricky. Our Jest API tests already perfectly satisfy the Level 1 "Test Gatekeeper" requirement.
